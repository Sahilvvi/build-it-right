    ...
    <SiteLayout>
      ...
      <section className="relative overflow-hidden bg-[hsl(220_50%_9%)] py-24 text-primary-foreground md:py-32">
        ...
        <div className="container-px relative mx-auto max-w-7xl">
          ...
          {/* Tabs with layoutId smooth pill */}
          <div className="mt-12 inline-flex flex-wrap gap-1 rounded-full border border-white/15 bg-white/5 p-1.5 backdrop-blur">
            {tabs.map((t) => (
              <button
                key={t}
                onClick={() => setActive(t)}
                className={cn(
                  ...
                  active === t ? "text-accent-foreground" : "text-white/70 hover:text-white"
                )}
              >
                {active === t && (
                  <motion.span
          ...
          <motion.div layout className="mt-10 grid gap-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
